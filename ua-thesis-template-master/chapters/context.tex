...
According to \citet{12_intel_tdx} achieves the previous using a combination of three components, namely, memory access control, runtime memory encryption and an 
...
and uses the SEAMCALL introduction to call host-side functions, functions that start with TDH, of the modulea. This is possible since the the logical processor 
changes \ac{vmx} root mode into \ac{seam} \ac{vmx} root mode and starts executing code within the TDX Module, upon completing it returns in \ac{vmx} root mode 
with the instruction SEAMRET. 
