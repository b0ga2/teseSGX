...
technical challenges associated with data processing within an enclave, concluding with known attacks to \ac{sgx}.
...
In the context of cloud computing and data analysis, areas particularly relevant to this dissertation, \ac{tee}s provide a critical layer of defense. 
...
Trusted Application (TA) running inside the enclave. 
...
The contents of this map can only be read by ab enclave as stated in Intel \ac{sdm}.
...
While \ac{tdx} shares the same high-level goals as AMD \ac{sev}, it is fundamentally built upon the trusted execution principles established by \ac{sgx}. 
Internally, it leverages similar architectural concepts, including the use of \texttt{ECALLs} transitions and the \ac{dcap} framework for remote attestation. 
...
entities in the environment, such as the hypervisor or neighboring \ac{vm}s in the same cloud tenant. Also tackling memory limitations imposed by \ac{sgx}.
