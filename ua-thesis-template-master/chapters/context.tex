...
After this, the discussion focuses on Intel \ac{sgx}, detailing its architecture, workflow, security mechanisms and
technical challenges associated with data processing within an enclave.
...
\ac{ap}s. 
These allow for the determination of user location with a reasonable degree of precision, as users must connect to a specific \ac{ap} 
to access the network.

Although often categorized as technical metadata, it presents privacy challenges due to its potential to uncover sensitive information. 
...
As \citet{04_article} states, the critical factor is the computational distance required to infer sensitive 
...
It is relevant to mention that the collection of connection logs is not driven by the production 
of occupancy analytics, but from a contractual obligation imposed on institutions, like the University of Aveiro, 
that implement the eduroam network. 
As stated in \citet{eduroam_compliance}, a \ac{sp} is required to maintain logging information for a minimum retention period of 
three months, although this duration may vary depending on national regulations. 
Specifically, this logging includes the timestamps of authentication requests, the outer \ac{eap} identity, 
the client's \ac{mac} address, and the identification of the specific \ac{ap} used for the connection, which will be used 
...
\begin{itemize}
    ...
    this serves as a fingerprint that uniquely identifies a user's device.
...
Using a combination of hardware and software components in the \ac{cpu}, it isolates specific workloads, commonly referred to as \ac{ta}, from the rest of the system.
...
As seen in the Figure \ref{fig:TEE_Building_Blocks} by \citet{01_7345265}, a TEE in composed by building blocks such as:
\begin{itemize}
...
this includes the traditional operating system, the hypervisor, and the standard application stack (yellow component of Figure \ref{fig:ree_vs_tee}). 
While the \ac{ree} is optimized for performance, versatility, and accessing peripherals, it loses the security guarantees 
...
A \ac{tee} technology provides protection for various applications by enforcing isolation and integrity, as mentioned in the systematic review by \citet{sok}, 
the scenarios for this technology appear in multiple domains, ranging from consumer electronics to critical cloud infrastructure.

In digital media, rights holders frequently employ \ac{tee}s for \ac{drm} to prevent the unauthorized copying of video or audio content, by establishing a hardware channel 
between the \ac{tee} and the device's display, the system blocks the device owner or malicious software from intercepting decoded media.

Financial applications also benefit from this technology, \ac{tee}s can be used to secure mobile wallets, peer-to-peer payments, and point-of-sale terminals. 
Furthermore, in blockchain ecosystems, they enable privacy for lightweight clients without compromising the performance of full nodes, acting as trusted backend systems to facilitate secure transactions.

In authentication \ac{tee}s are used to protect biometric identity methods such as facial recognition and fingerprint sensors. 
For example, the Android \ac{os} stores fingerprint data within the \ac{tee}, making it inaccessible to the \ac{os} environment. 
This isolation capability is also utilized to support passwordless standards, such as Apple’s passkeys, by combining biometric verification with securely stored cryptographic private keys.

In the context of cloud computing and data analysis, areas relevant to this dissertation, \ac{tee}s provide a critical layer of defense. 
In a trusted cloud model, they protect the infrastructure against compromise, even if the backend or host \ac{os} is malicious, the adversary remains unable to access the specific 
\ac{ta} running inside the enclave. 
This is also common for \ac{ml} in sensitive fields like healthcare, allowing computation on confidential datasets without exposing the underlying information.

Finally, \ac{tee}s also contribute to system stability through runtime integrity and secure modular programming. 
They can enforce real-time kernel protection by isolating security monitoring services, potentially shutting down systems if kernel binaries are targeted. 
Additionally, they encourage secure software architectures by decoupling functionalities into self-contained modules, where the execution of one module is protected from the vulnerabilities of others.
...
This chapter provides a comparative overview of these technologies, however, Intel \ac{sgx}, the focus of this dissertation, is reviewed in greater detail in the subsequent section. 
Intel \ac{tdx} is examined thereafter, as it represents the natural evolution of the \ac{sgx} architecture.
...
adoption within the community, particularly in the context of \ac{rd} in \ac{iot} devices.

While TrustZone is presented the standard for \ac{tee}s in the embedded ecosystem, its adoption in this work is constrained by the architectural model of the proposed solution. 
The current approach focuses on the centralized processing of aggregated connection logs exported by the network infrastructure, rather than computing directly on the edge devices. 
Deploying custom trusted applications directly onto the university's \ac{ap}s is impractical, as it would require proprietary access to the firmware and administrative privileges, 
or the acquisition of specific hardware that would lead to additional costs.

\subsection{AMD SEV}

Introduced in 2016, \ac{sev} is stated to be the first x86 technology that was designed to isolate a \ac{vm} from the hypervisor, since 
...
The most recent version is the \ac{sev-snp}, built on top of the previous versions while adding hardware based protections like 
memory integrity protection to prevent attacks like data replay, memory re-mapping, it also implements 
protection in interrupt behavior. 
...
\ac{vmpl} to securely offer services like virtual \ac{tpm} and live migration.

This approach was not selected for this implementation due to architectural and practical constraints, AMD \ac{sev} operates at the \ac{vm} level, 
encrypting the entire guest memory, even though this mitigates the memory capacity limitations inherent to \ac{sgx}, 
it significantly expands the \ac{tcb} to include the entire guest \ac{os}, increasing the attack surface. 
In contrast, \ac{sgx} offers application-level isolation, excluding the \ac{os} from the trust boundary, which is more aligned with the goal of 
developing a lightweight solution.
...
In simpler terms, this model relies on software attestation, a capability that allows the hardware to prove to a user that they are 
...
the presence of sensitive information regarding the enclave.
...
%Mencionar atributos
While the \ac{elrange} defines the memory boundaries of the enclave, the nature of its execution environment is defined by the \texttt{ATTRIBUTES} field within the \ac{secs}, 
...
texttt{INIT} attribute to false, while in this state, the lifecycle enters the loading phase, where the system software uses \texttt{EADD} to allocate pages for code and data, 
...
Finally, the destruction of the enclave occurs with the \texttt{EREMOVE} instruction, which deallocates pages by clearing their \texttt{VALID} bit, ensuring that the \ac{secs} page is only destroyed once all other 
...
of \ac{sgx} instructions to allocate and free pages in the enclaves. But as mentioned previously, this is not a trusted component, so the \ac{sgx} processor verifies every allocation decision, 
...
Three relevant fields in each entry that affect the ownership of the \ac{epc} page are \texttt{VALID}, a bit indicating whether an \ac{epc} page is currently active and allocated to an enclave, if 
...
The contents of this map can only be read by an enclave as stated in Intel \ac{sdm}.

\subsection{ECALLs and OCALLs}
%Mecanismo de ECalls e OCalls (entrar e sair do enclave e comunicação com o mundo "inseguro").
In Intel \ac{sgx}, \texttt{ECALL} and \texttt{OCALL} are mechanisms for communication between the untrusted application and the trusted enclave. 

\texttt{ECALLs} allow the untrusted application to invoke a pre-defined function inside the enclave, passing input parameters and pointers to shared memory within the application.

On the other hand, \texttt{OCALLs} enable the enclave to invoke a pre-defined function in the untrusted application, but unlike \texttt{ECALLs}, 
cannot share enclave memory directly and must copy parameters into the application's memory before the call.

Both \texttt{ECALLs} and \texttt{OCALLs} are defined using the \ac{edl}, where \texttt{ECALLs} are declared in the trusted section and \texttt{OCALLs} in the untrusted section of the file.
...
The first, \texttt{MRENCLAVE}, contains a hash that uniquely identifies the enclave's code and data contents, acting as a cryptographic log of the creation process, 
...
This register stores the hash of the public key modulus used to sign the \texttt{SIGSTRUCT} certificate, allowing remote parties to trust an 
enclave based on the author's reputation. 
The figure \ref{fig:enclave_identity} shows how an enclave identity is established.

\newpage
\begin{figure}
  \begin{center}
  \includegraphics[scale=0.6]{figs/enclave_identity.png}
...
To extend this trust remotely, \ac{sgx} uses a privileged enclave known as the Quoting Enclave, that receives the local report, verifies it, and replaces the local \ac{mac} with a 
...
\section{Memory and Performance Limitations in \ac{sgx}}
...
In addition to memory capacity, the cost of entering (\texttt{ECALL}) and exiting (\texttt{OCALL}) an enclave is worth mentioning, since a 
domain transition involves several hardware operations, including flushes, security checks, and context saving, as shown in the figure \ref{fig:enclave_overhead}. 

\begin{figure}
  \begin{center}
  \includegraphics[scale=0.35]{figs/overhead_calls_sgx.png}
  \caption{Performance Overhead of an Enclaves as a Function of Data Size, by \citet{sgx_overhead}.}
  \label{fig:enclave_overhead}
  \end{center}
\end{figure}

This happens since \ac{sgx} performance is dependent on the working set size, 
for data sizes under 64MB, the overhead remains manageable, however, a performance bottleneck is observed beyond 96MB. 

This threshold corresponds to the physical limit of the \ac{epc}, once the enclave's memory usage exceeds this capacity, the \ac{os} 
is forced to perform secure paging, to swap them to untrusted memory. 
This results in a performance degradation, with execution times reaching up to 500 times that of native code due to the use of more clock cycles, 
showing the need for optimized memory management.

\subsection{Intel TDX}

Representing a natural evolution of the \ac{sgx} architecture, Intel \ac{tdx} expands the concept of hardware-enforced isolation from 
individual application enclaves to entire virtual environments, introduced with the 4th Generation Intel Xeon Scalable processors.  
This technology facilitates the deployment of confidential \ac{vm}s, referred to as \ac{td}s, under the supervision of the \ac{seam}, that provides encrypted \ac{cpu} 
state and memory, with integrity protection and remote attestation, addressing scenarios where the host platform is considered 
untrustworthy within the threat model
\footnote{Structured approach to identifying potential security risks, defining attacks from the perspective of 
an adversary to strengthen system defenses \citep{threat_model}.}. 

While \ac{tdx} shares the same high-level goals as AMD \ac{sev}, it is built upon the trusted execution principles established by \ac{sgx}. 
Internally, it uses similar architectural concepts, including the use of \texttt{ECALLs} transitions and the \ac{dcap} framework for remote attestation. 

Technically, \ac{tdx} combines these \ac{sgx} foundations with virtualization technologies from other Intel technologies such as \ac{vt} and \ac{mktme}, together, 
these components enforce the confidentiality and integrity of \ac{td} memory and \ac{cpu}, ensuring that they can neither be accessed nor tampered with by other 
entities in the environment, such as the hypervisor or neighboring \ac{vm}s in the same cloud tenant.
...
and uses the \texttt{SEAMCALL} introduction to call host-side functions, functions that start with "TDH", of the module. 

This is possible since the the logical processor changes \ac{vmx} root mode into \ac{seam} \ac{vmx} root mode and starts executing code within the \ac{tdx} Module, 
upon completing it returns in \ac{vmx} root mode with the instruction \texttt{SEAMRET}. 
...
\ac{vm}, but the guest \ac{os} kernel (TDX-enlightened OS in Figure \ref{fig:intel_tdx}), needs to be altered in order to align with the \ac{tdx} platform, namely 
...
Although this technology effectively tackles the memory limitations imposed by \ac{sgx}, eliminating the need for manual memory management in large data processing, 
its adoption in this work is constrained by current hardware availability. 
According to Intel specifications
\footnote{https://www.intel.com/content/www/us/en/support/articles/000091103/processors/intel-xeon-processors.html}
, \ac{tdx} requires 4th Generation Intel Xeon Scalable Processors, or newer, alongside specific platform support. 
Given that the computational infrastructure available for the development of this dissertation relies on earlier hardware generations that do not possess these extensions, 
the focus remains on \ac{sgx}. 