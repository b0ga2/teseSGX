...
to access the network, and every attempt, successful or not, is logged.

Although often categorized as technical metadata, it presents privacy challenges when observed through the analysis of sensitive data. 
...
As stated in \citet{eduroam_compliance}, \ac{sp}s are required to maintain logging information for a minimum retention period of 
...
Introduced in 2016, \ac{sev} is stated to be the first x86 technology that was designed to isolate \ac{vm}s from the hypervisor, since 
...
and uses the \texttt{SEAMCALL} introduction to call host-side functions, functions that start with TDH, of the modulea. 
...
These limitations are derived from the restricted size of the secure memory region and the cost of transitioning between trusted and 
...
domain transition involves several hardware operations, including flushes, security checks, and context saving. 
