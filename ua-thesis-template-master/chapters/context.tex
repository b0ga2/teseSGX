...
After this, the discussion focuses on Intel \ac{sgx}, detailing its architecture, workflow, and security mechanisms, concluding with
the technical challenges of data processing in an enclave.
...
less suitable for the available infrastructure, where x86 architecture are more common.
...
for integrity and confidentiality during computation in a computer where the all the privileged components might be compromised, as defined by \citet{sgx}.
...
In the \ac{prm} also is the \ac{epc}, which consists of 4KB memory pages that store the enclave's code and data. 
...
At this point the loaded code becomes executable in the enclave and the loading method is dissabled to prevent modifications. 
...
When an application's working set exceeds the available \ac{epc} capacity, the system must perform paging to evict \ac{epc} pages to 
unprotected \ac{dram}. 
