...
The domain of privacy in data processing is made of several methodologies, ranging from syntactic models like k-anonymity to semantic 
...
%\subsection{Secure Data Analytics}
%01
%03
%\subsection{Secure Database Management}
%02
% New additions
%\subsection{Privacy-Preserving \ac{ML}}
%04
%06
%07
%08
...
%15
%16
%\subsection{Privacy-Preserving Data Analysis}
%05
%\subsection{Trusted Cloud Computing}
%11
...
Recognizing that the lack of trusted \ac{gpu} support in SGX creates a significant bottleneck for heavy matrix operations, the authors introduced a hybrid execution model. 
...
Instead of processing the entire computation inside the \ac{tee}, which would exceed the available secure memory of the \ac{epc}, DarkneTZ employs a model partitioning strategy. 
...
Instead of processing the entire dataset within the enclave, GOAT delegates the intensive matrix operations of forward and backward propagation to an untrusted \ac{gpu}. 
...
This mechanism partitions the data processing workflow into a parameter servers architecture, where the computational load is distributed across multiple enclaves, workers compute gradients on private data shards, while parameter servers aggregate these updates to refine the global model. 
To handle the intensive memory demands of tensor operations that typically exceed the trusted memory of the \ac{epc}, the system uses a shielded execution runtime, in this case SCONE, to manage secure paging and encrypted I/O transparently during execution. 
...
\citet{s10} addressed the performance bottleneck of aggregating gradient vectors within \ac{sgx} by modifying the data processing logic to exploit sparsity. 
...
While frameworks like DarkneTZ address memory limitations by offloading layers to untrusted hardware, \citet{s12} proposed Twine, a runtime system designed to keep the inference execution within the enclave despite the limited \ac{epc} size. 
Identifying that the standard execution flow of deep learning frameworks triggers excessive and costly \ac{os}-managed paging, Twine modifies the data processing strategy through tensor partitioning and tiling. 
Instead of processing full layers, the system breaks the input tensors and model weights into smaller spatial blocks that fit within the enclave's physical memory. 
Coupled with a static memory planner that aggressively recycles buffers throughout the execution graph, this block-wise processing strategy ensures that the working set remains resident in the secure cache, effectively eliminating the performance penalty associated with page swapping.
...
\citet{s13} proposed PPFL, a framework that shifts the processing from cryptographic computation to hardware-based isolation. 
...
In scenarios that require local training on devices with low resources, such as federated learning clients, memory limitations are even more noticeable due to the necessity of storing states. 
\citet{s14} addressed this by restructuring the training algorithm, proposing a layer training strategy. 
Instead of performing a overall update which requires simultaneous memory allocation for the entire model's gradients and activations, the system decomposes the training sequentially. 
It loads and updates only a single layer within the \ac{tee} at a time, while freezing the parameters of the remaining network. 

% The following is regarding the article number 15 in the improvements folder
Addressing the vulnerability of gradient linkability, \citet{s15} introduced a shuffling mechanism within the \ac{tee} processing pipeline. 
...
Incoming gradients are buffered and subjected to a random permutation inside the enclave, effectively decoupling the gradient values from their identifiers, 
this ensures unlinkability, preventing the server from correlating specific model updates with their originating clients based on communication metadata or execution timing.
...
To address this, the authors proposed Garrus, a framework that reconciles efficiency with security through oblivious sparse aggregation, so 
