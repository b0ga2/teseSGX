...
Complementing the previous discussion on performance trade-offs, \citet{s05} argued that data-oblivious techniques, while adding security guarantees, 
...
Recognizing that the lack of trusted GPU support in SGX creates a significant bottleneck for heavy matrix operations, the authors introduced a hybrid execution model. 
In this approach, the enclave delegates work to a remote or local GPU to use its processing power, but this delegation does not align with the confidential necessities, so to ensure security, 
it uses a verification mechanism based on Freivalds' algorithm, that allows the \ac{tee} to check the integrity of the GPU's output without re-executing the full computation. 
For privacy-preserving inference, the system utilizes input blinding techniques, ensuring that the data processed by the untrusted GPU remains opaque, this decouples 
...
In parallel with hardware offloading, \citet{s07} addressed the specific challenge of executing \ac{dnn} within the memory-constrained environments of TEEs. 
Placing an \ac{ml} model inside a \ac{tee} will likely exceed the available secure memory, the authors proposed DarkneTZ, a framework based on model partitioning, 
this splits the \ac{dnn} execution, only the most sensitive layers are executed within the \ac{tee}, while the remaining heavy layers execute in the untrusted operating system. 
This hybrid approach minimizes the \ac{tee} memory footprint and execution overhead, while maintaining privacy guarantees against inference attacks. 
This strategy highlights a distinct architectural trade-off: unlike Slalom, which offloads for performance, DarkneTZ partitions for memory efficiency, 
a consideration for \ac{sgx} implementations limited by the \ac{epc} size.
