...
In 2021, \citet{01_ALISHAHI2021107936} proposed a framework that utilizes \ac{ml} and statistical analysis to extract key occupancy indicators from simple WiFi 
...
The framework yielded positive results, however, \citet{01_ALISHAHI2021107936} also highlighted critical limitations that directly impact the design of this thesis's methodology. 
...
Regarding it's capabilities, the study emphasized the potential of the \ac{ieee} 802.11mc standard, which allows for precise 
...
\citet{03_WANG2019228} focused on the technical feasibility of detecting inactive smartphones to improve zone detection accuracy. 
...
require reliance on a \ac{ttp}, which introduces a single point of failure.
In contrast, their work demonstrates that SGX enables a lightweight, integrated privacy scheme, by processing sensitive location data within a hardware protected 
...
Encrypted data is ingested in manageable input splits, processed within the enclave's isolated heap, and the results are immediately encrypted and streamed out. 
To mitigate the performance overhead of \ac{sgx} context switches, between ECalls and OCalls, \ac{vc3} utilizes a shared memory region for communication and implements aggressive batching. 
By processing key value pairs in batches, rather than individually, the system significantly reduces the frequency of expensive enclave transitions.
