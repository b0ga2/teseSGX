...
\section{Data Processing in SGX}
...
The authors stated that although SGX allows for memory confidentiality, standard \ac{ml} algorithms show data-dependent memory access patterns that can be observed by a 
...
This ensures that the execution trace depends only on public parameters, such as the dataset size, rather than the input data. 
...
These dummy records are then shuffled with the real user data using a sorting network based on random keys generated via \ac{sgx}'s hardware random number generator function. 
...
This introduces a trade-off between privacy and efficiency, with the determinism of data-obliviousness in favor of a probabilistic protection model, 
the system achieves higher computational throughput, making it more suitable for large-scale data analytics tasks.
...
Addressing the hardware limitations of executing \ac{dnn} within \ac{cpu}-bound enclaves, in 2019 \citet{s06} proposed Slalom, a framework for verifiable and private execution. 
...
Within the enclave, the processing logic includes a data sanitization step, by normalizing the output confidence scores, before returning them to the untrusted \ac{os}. 
...
The processing pipeline is controlled by a hardware-based security manager, which performs decryption and encryption of data streams entering and leaving the processing modules. 
...
Instead of processing full dense updates, which often triggers page thrashing in the \ac{epc}, the authors implemented a Top-$k$ gradient selection algorithm directly within the enclave. 
This technique filters incoming data, aggregating only the most significant gradients while discarding negligible values, thereby reducing the memory working set. 
Furthermore, the system optimizes the remaining computation through block-wise data loading and double buffering, ensuring that the processing of gradient blocks aligns with the \ac{cpu} cache size to minimize memory access latency during the aggregation phase.

% The following is regarding the article number 09 in the improvements folder
Scaling data processing beyond the limits of a single trusted enclave, \citet{s09} introduced SecureTF, a mechanism for distributed \ac{ml} execution. 
...
, to manage secure paging and encrypted \ac{io} transparently during execution. 
This allows for the stateful processing of large-scale datasets and complex computation graphs across a cluster of \ac{tee}s, ensuring that data remains encrypted not only at rest but also throughout the entire distributed calculation pipeline.
...
\citet{s18} introduced Veracruz, a framework designed to allow secure delegated computations. 
The central feature of this architecture is the utilization of a WebAssembly environment within the enclave, which allows the execution of arbitrary 
programs defined in various languages. 
This design establishes a dual layer, combining the hardware-enforced protection of the \ac{sgx} enclave with the software-based sandboxing of the WebAssembly runtime. 
Functionally, the system acts as a trusted component on the untrusted host, enabling multiple mutually independent parties to 
provision both input data and computational modules via remotely attested channels. 
By relying on the runtime binary format, the framework abstracts the hardware complexity and ensures that the data processing logic 
strictly confined within the secure boundary.
